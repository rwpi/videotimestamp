...
VIDEO_QUEUE_EXTENSIONS = {".mts", ".mp4", ".mov"}
...
class TimestampPanel(QWidget):
    ...
    def set_details_panel(self, panel):
        self.layout.insertWidget(0, panel, 1)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.timestamp_panel = None
        self.video_renamer_window = None
        self.merge_clips_window = None
        self.video_minutes_counter_window = None
    ...
    def _setup_module_panels(self):
        ...
        self.import_panel.import_button.clicked.connect(self.handle_import_button)
        self.module_stack.addWidget(self.import_panel)
        self.timestamp_panel = TimestampPanel(self)
        self.timestamp_panel.process_button.clicked.connect(self.handle_process_button)
        self.timestamp_panel.set_details_panel(self.queue_panel)
        self.module_stack.addWidget(self.timestamp_panel)
        self.module_stack.setCurrentWidget(self.import_panel)

    def launch_video_renamer(self):
        from video_renamer_gui import MainWindow as VideoRenamerWindow
        if self.video_renamer_window is None:
            ...
            self.video_renamer_window.status_changed.connect(self.on_renamer_status_changed)
            ...
            self.module_stack.addWidget(self.video_renamer_window)
        self._sync_module_folder_paths()
        self.module_stack.setCurrentWidget(self.video_renamer_window)
        self.set_active_nav_button(self.rename_button)

    def launch_merge_clips(self):
        from merge_clips_gui import MainWindow as MergeClipsWindow
        if self.merge_clips_window is None:
            initial_folder = self.output_folder_path or None
            self.merge_clips_window = MergeClipsWindow(initial_folder=initial_folder)
            self.merge_clips_window.run_started.connect(self.on_merge_started)
            self.merge_clips_window.status_changed.connect(self.on_merge_status_changed)
            self.merge_clips_window.progress_changed.connect(self.update_merge_progress)
            self.merge_clips_window.run_finished.connect(self.on_merge_finished)
            self.module_stack.addWidget(self.merge_clips_window)
        self._sync_module_folder_paths()
        self.module_stack.setCurrentWidget(self.merge_clips_window)
        self.set_active_nav_button(self.merge_button)

    def launch_video_minutes_counter(self):
        from video_minutes_counter_gui import MainWindow as VideoMinutesCounterWindow
        if self.video_minutes_counter_window is None:
            initial_folder = self.output_folder_path or None
            self.video_minutes_counter_window = VideoMinutesCounterWindow(initial_folder=initial_folder)
            self.module_stack.addWidget(self.video_minutes_counter_window)
        self._sync_module_folder_paths()
        self.module_stack.setCurrentWidget(self.video_minutes_counter_window)
        self.set_active_nav_button(self.total_counter_button)

    def show_import_panel(self):
        if self.import_panel is None:
            return
        self.module_stack.setCurrentWidget(self.import_panel)
        self.set_active_nav_button(self.import_button)

    def show_timestamp_panel(self):
        if self.timestamp_panel is None:
            return
        self.module_stack.setCurrentWidget(self.timestamp_panel)
        self.set_active_nav_button(self.process_button)
    ...
    def set_active_nav_button(self, active_button):
        for button in (
            ...
            self.rename_button,
            self.merge_button,
            self.total_counter_button,
        ):
            button.setChecked(button is active_button)

    def _sync_module_folder_paths(self):
        folder = self.output_folder_path or ""
        for module in (
            self.video_renamer_window,
            self.merge_clips_window,
            self.video_minutes_counter_window,
        ):
            if module is not None and hasattr(module, "_set_folder_path"):
                module._set_folder_path(folder)
    ...
    def choose_output_folder(self):
        ...
        if self.output_folder_path:
            ...
            self.output_folder_label.setText(f'Output Folder: <a href="#">{folder_name}</a>')
    ...
    def choose_input_files(self):
        ...
        if new_input_files:
            self.add_input_files(new_input_files)
    ...
    def add_input_files(self, paths):
        ...
        for path in paths:
            ...
            if not resolved.is_file():
                continue
            if resolved.suffix.lower() not in VIDEO_QUEUE_EXTENSIONS:
                continue
            valid_files.append(str(resolved.resolve()))

        if not valid_files:
            return

        self.input_files.update(valid_files)
        self.refresh_input_file_queue()
        self.check_if_ready_to_process()

    def refresh_input_file_queue(self):
        self.input_files_label.setText(f"{len(self.input_files)} Input Files Selected")
        self.input_files_list.clear()
        for file in sorted(self.input_files, key=lambda item: os.path.basename(str(item)).lower()):
            file_path = str(file)
            self.input_files_list.addItem(os.path.basename(file_path))
            item = self.input_files_list.item(self.input_files_list.count() - 1)
            item.setData(Qt.UserRole, file_path)

    def remove_selected_input_files(self):
        selected_files = {
            item.data(Qt.UserRole)
            for item in self.input_files_list.selectedItems()
            if item.data(Qt.UserRole)
        }
        if not selected_files:
            return
        self.input_files = {file for file in self.input_files if str(file) not in selected_files}
        self.refresh_input_file_queue()
        self.check_if_ready_to_process()
    ...
    def check_if_ready_to_process(self):
        can_process = bool(self.input_files and self.output_folder_path)
    ...
    def on_worker_finished(self):
        self.timer.stop()
        was_cancelled = getattr(self.worker, "was_cancelled", False)
        self.is_processing = False
        self.status_label.setText("Process Video Cancelled" if was_cancelled else "Timestamping Complete")
    ...
    def cancel_processing(self):
        if not self.is_processing or not hasattr(self, "worker"):
            return
        self.status_label.setText("Cancelling Process Video")
        if self.timestamp_panel is not None:
            self.timestamp_panel.process_button.setText("Cancelling...")
            self.timestamp_panel.process_button.setEnabled(False)
        self.worker.stop()
    ...
    def begin_import(self):
        selected_date, destination_base, skip_duplicates = self.import_panel.get_values()
        ...
        self.progress_bar.setValue(0)
        self.import_panel.import_button.setText("Cancel")
        self.import_panel.import_button.setEnabled(True)
        self.import_thread.finished.connect(self.set_output_folder)
        self.import_thread.finished.connect(self.add_new_files)

    def cancel_import(self):
        if not self.is_importing or not hasattr(self, "import_thread"):
            return
        self.status_label.setText("Cancelling Import")
        self.import_panel.import_button.setText("Cancelling...")
        self.import_panel.import_button.setEnabled(False)
    ...
    def on_renamer_status_changed(self, text):
        suppressed_prefixes = (
            "Organizing detected unprocessed LawMate files",
            "Skipped ",
            "Lawmate:",
        )
        if text.startswith(suppressed_prefixes) or "non-LawMate" in text:
            return
    ...
    def on_merge_status_changed(self, text):
        self.status_label.setText(text)
    ...
    def finish_import(self):
        was_cancelled = getattr(self.import_thread, "was_cancelled", False)
        self.is_importing = False
        self.import_panel.import_button.setText("Import Files")
        self.import_panel.import_button.setEnabled(True)
        self.status_label.setText("Import Cancelled" if was_cancelled else "Import Complete")
        if not was_cancelled:
            self.progress_bar.setValue(100)
        self.progress_bar.setFormat("%p%")
    ...
    def add_new_files(self, folder_path, new_files):
        ...
        self.input_files.update(str(Path(file).expanduser().resolve()) for file in new_files)
        self.refresh_input_file_queue()
        self.check_if_ready_to_process()
